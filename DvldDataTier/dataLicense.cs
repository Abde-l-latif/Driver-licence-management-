...
namespace DvldDataTier
{
    public class dataLicense
    {
        ...
        static public void getDetainDetails(int LicenseID, ref int DetainID , ref DateTime DetainDate , ref Decimal FineFees)
        {
            string query = "select DetainID , DetainDate , FineFees from DetainedLicenses where LicenseID = @LicenseID;";
        ...
        static public int ReleaseDetainedLicense(int LicenseID, bool IsReleased, DateTime ReleaseDate, int UserID, int AppID)
        {
            int EffectedRows = 0;

            string Query = @"UPDATE DetainedLicenses set IsReleased = @IsReleased , ReleaseDate = @ReleaseDate ,
            ReleasedByUserID = @UserID, ReleaseApplicationID = @AppID where LicenseID = @LicenseID;";
        ...
        static public DataTable getFiltredData(string text, string ComboText)
        {

            string Filter = ComboText == "Detain ID" ? "T.DetainID" :
                            ComboText == "National No" ? "T.NationalNo" :
                            ComboText == "Full Name" ? "T.FullName" :
                            ComboText == "Release Application ID" ? "T.ReleaseAppID" :
                            ComboText == "Is Released" ? "T.IsReleased" : null;
            ...
            string Query = $@"
                            ...
                            inner join Licenses L on D.LicenseID = L.LicenseID
                            inner join Drivers DR on DR.DriverID = L.DriverID
                            inner join People P on DR.PersonID = P.PersonID ) T
                            where {Filter} like @text;";
